...
analysis.dist = analysis.dist[analysis.dist$pscore > 0]
...
hte.chart <- function(outcome, legend.pos, label, controls){
  ...
  plot <- 
    ...
    labs(x = "District Hispanicity",
         ...
         subtitle = "TWFE Outcome Model with Random Forest Treatment Model Trimming for 0%",
         caption = paste0("*Note:* Implementation of RIPW estimator for the DATE, see Arkhangelsky et al. (2023). Red line and interval is aggregate DATE. <br> Tau = ",
                          ...
                          ". Districts grouped by being above or below the treatment unit median Hispanic <br> population share or dissimilarity index. Estimate corresponding to the right-hand-size blue bars is 0.267, se 0.148. <br> Estimate corresponding to the left-hand-side blue bars is 0.207, se 0.103. Pooled estimate with high segregation is <br> 0.093 se 0.055. This is unusual behaviour and representative of stability issues with 0% score trimming.")) +
...
pdf(file = "../../output/ripw_robust/stability_0_pc.pdf", width = 7.9, height = 4.9)
hte.chart("Prop.Ran.Hisp.C", c(0.9, 1), "Proportion Board Elected Hispanic", 1)
dev.off()
