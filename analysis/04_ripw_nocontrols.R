...
analysis.dist = analysis.dist[analysis.dist$pscore > .025]
...
hte.chart <- function(outcome, legend.pos, label, controls){
  ...
  plot <- 
    ...
    labs(x = "District Hispanicity",
         ...
         subtitle = "TWFE Outcome Model with Random Forest Treatment Model Without Controls",
         caption = paste0("*Note:* Implementation of RIPW estimator for the DATE, see Arkhangelsky et al. (2023). Red line and interval is aggregate DATE. <br> Tau = ",
                          ...
                          ". Districts grouped by being above or below the treatment unit median Hispanic <br> population share or dissimilarity index. This version presents results without controls.")) +
...
pdf(file = "../../output/ripw_robust/prop_ran.pdf", width = 7.9, height = 4.9)
hte.chart("Prop.Ran.Hisp.C", c(0.9, 1), "Propotion Candidates Ran Hispanic", 1)
dev.off()

pdf(file = "../../output/ripw_robust/prop_elec.pdf", width = 7.9, height = 4.9)
hte.chart("Prop.Win.Hisp.C", c(0.9, 1), "Proportion Board Elected Hispanic", 1)
dev.off()

pdf(file = "../../output/ripw_robust/sfp_bin.pdf", width = 7.9, height = 4.9)
hte.chart("Dist.SFP.Binary", c(0.9, 1), "Binary Indicator for SFP Project", 1)
dev.off()

pdf(file = "../../output/ripw_robust/eld_rt.pdf", width = 7.9, height = 4.9)
hte.chart("Dist.RT.EL.FT", c(0.9, 1), "Full Time English-Language Teacher-Student Ratio", 1)
dev.off()

pdf(file = "../../output/ripw_robust/g_eld.pdf", width = 7.9, height = 4.9)
