...
# Full districts
x = list()
for(i in 1:nrow(frame.dist)){
  x[[i]] = census.data(frame.dist[i,])
}

census.districts = bind_rows(x)

# Remove Extranous Before Writing ----------------------------------------------

frame.sch <- st_drop_geometry(frame.sch)
frame.sch <- dplyr::select(frame.sch, -BG.List, -geometry)


# Write ------------------------------------------------------------------------
write.csv(
  frame.sch,
  "../../data/output/analysis/census_school.csv"
)

write.csv(
  census.districts,
  "../../data/output/analysis/census_district.csv"
)
